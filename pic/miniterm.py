...
class Miniterm:
    def __init__(self, port, baudrate, parity, rtscts, xonxoff, echo=False, convert_outgoing=CONVERT_CRLF, repr_mode=0):
        ...
        self.serial = serial.Serial(port, baudrate, parity=parity, rtscts=rtscts, xonxoff=xonxoff, timeout=0.7, bytesize = 7)
        self.echo = echo
    ...
    def sendCommand( self, cmd ):
        cmd = 0x30
        c = struct.pack( "BBBB", ord( '0' ), ord( '1' ), cmd, 3 )
        #cs = 0
        #for x in c:
        #    cs = cs ^ ord( x )
        #cs = sum( ord( x ) for x in c ) 
        for cs in range( 0, 255 ):
            d = chr( 2 ) + c + chr( cs )
            time.sleep( 1 )
            
            print "Writting %s" % ( ", ".join( "0x%x" % ord( x ) for x in d ) ) 
            self.serial.write( d )
    ...
    def writer(self):
        ...
        while self.alive:
            try:
                ...
                c = '\x03'
            if c == SEND_TEXT: 
                self.serial.write( "0123456789abcdefghijklmnopqrstuvwxyz" + self.newline )  # send newline character(s)
            elif c == EXITCHARCTER: 
                self.stop()
                break                                   # exit app
